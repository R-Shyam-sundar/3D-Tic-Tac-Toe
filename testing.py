...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.pushButton.clicked.connect(lambda : self.submit_form(ai, cells_lst))
    ...
    def submit_form(self, ai, cells_lst):
        ...
        print("name:____{}____".format(name))  
        if selection != "  you":
            tup = ai.choice(2)
            if tup[0] == -1 and tup[1] == -1 and tup[2] == -1:
                return
            ...
            for j in range(0,3):
                for k in range(0,3):
                    if self.tboard[i][j][k] == '*':
                        self.tboard[i][j][k] = 'o'
                        cval = self.min_node(alpha,depth+1)
                        if cval > alpha:
                            x = i;y = j;z = k;
                            alpha = cval
                        self.tboard[i][j][k] = '*'
    ...
    def min_node(self,alpha,depth):
        ...
        for i in range(0,3):
            for j in range(0,3):
                for k in range(0,3):
                    if self.tboard[i][j][k] == '*':
                        print("Alpha = {}, Beta = {}, Depth = {}".format(alpha,beta,depth))
                        self.tboard[i][j][k] = 'x'
                        cval = self.max_node(depth+1)
                        if cval < beta:
                            beta = cval
                            x = i;y = j;z = k;
                        self.tboard[i][j][k] = '*'
        ...
        for i in range(0,3):
            for j in range(0,3):
                for k in range(0,3):
                    if self.tboard[i][j][k] == '*':
                        self.tboard[i][j][k] = 'o'
                        cval = self.min_node(alpha,0)
                        if cval > alpha:
                            x = i;y = j;z = k;
                            print("x = {}, y = {}, z = {}, cval = {}".format(x,y,z,cval))
                            alpha = cval
                        self.tboard[i][j][k] = '*'
